...
const getInventory = async (req, res) => {
  ...
  const numberOfHorses = stablesInventory.reduce(
    (total, stablesSchema) => total + stablesSchema.numberOfHorses,
    0
  );
...
const orderOne = async (req, res) => {
  ...
  const existingOrder = await stablesOrderSchema.findOne({
    horseId: orderReq.horseId,
  });

  if (existingOrder) {
    const error = new DuplicateError(
      `Horse with ID ${orderReq.horseId} already exists`
    );
    throw error;
  }
...
const updateOne = async (req, res) => {
  ...
  const existingStable = await stablesSchema.findOne({
    $and: [{ name }, { location }, { owner }, { numberOfHorses }],
  });

  if (existingStable && existingStable.id.toString() !== id) {
    const error = new DuplicateError(
      `This update creates a duplicate of another saved stable data`
    );
    throw error;
  }
  ...
  if (existingStable && existingStable._id.toString() == id) {
    const error = new DuplicateError(
      `The data you entered has the same data to the original one`
    );
    throw error;
  }
