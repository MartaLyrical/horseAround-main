...
const createDiet = async (req, res) => {
    ...
    const response = await mongoClient.getDb().db().collection('diet').insertOne(diet);
    if (response.acknowledged) {
        res.status(201).json(response);
    } else {
        res.status(500).json(response.error || 'An error happened while creating this record.');
    }
};
...
const updateDiet = async (req, res) => {
    if (!ObjectId.isValid(req.params.id)) {
      ...
      const response = await mongoClient.getDb().db().collection('diet').replaceOne({ _id: dietId }, diet);
...
const deleteDiet = async (req, res) => {
    ...
    const response = await mongoClient.getDb().db().collection('diet').deleteOne({ _id: dietId }, true);
    console.log(response);
    if (response.deletedCount > 0) {
        res.status(204).send();
    } else {
        res.status(500).json(response.error || 'An error has occurred while deleting this record.');
    }
};
